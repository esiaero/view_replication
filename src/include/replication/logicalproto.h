...
typedef enum LogicalRepMsgType
{
	...
	LOGICAL_REP_MSG_REFRESHDATA = 'f', /* R and r both taken */ 
...
extern const char *logicalrep_read_ddlmessage(StringInfo in, XLogRecPtr *lsn, const char **prefix,
											  ...
											  Size *sz);
extern void logicalrep_write_refreshdata(StringInfo out, TransactionId xid, XLogRecPtr lsn,
								  Oid rel, bool concurrent, bool skipData,
								  bool isCompleteQuery,
								  const char *message,
								  Size sz);
extern LogicalRepRelId logicalrep_read_refreshdata(StringInfo in, bool *concurrent, bool *skipData,
								bool *isCompleteQuery,
								const char **message,
								Size *sz);
