 ...
 *
 * IDENTIFICATION
 *	  src/backend/access/rmgrdesc/refreshmsgdesc.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
void
logicalrefreshmsg_desc(StringInfo buf, XLogReaderState *record)
{
	...
	{
		xl_logical_refresh_message *xlrec = (xl_logical_refresh_message *) rec;
...
const char *
logicalrefreshmsg_identify(uint8 info)
{
	if ((info & ~XLR_INFO_MASK) == XLOG_LOGICAL_REFRESH_MESSAGE)
