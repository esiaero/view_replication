...
getPublications(Archive *fout, int *numPublications)
{
	...
	if (fout->remoteVersion >= 150000)
		appendPQExpBuffer(query,
						  ...
						  "p.pubowner, "
						  "p.puballtables, p.pubinsert, p.pubupdate, p.pubdelete, p.pubtruncate, p.pubviaroot, p.pubddl_database, p.pubddl_table, p.pubrefresh, p.pubrefresh_data "
						  "FROM pg_publication p");
	else if (fout->remoteVersion >= 130000)
		appendPQExpBuffer(query,
						  "SELECT p.tableoid, p.oid, p.pubname, "
						  "p.pubowner, "
						  "p.puballtables, p.pubinsert, p.pubupdate, p.pubdelete, p.pubtruncate, p.pubviaroot, false AS pubddl_database, false AS pubddl_table, false AS pubrefresh, false AS pubrefresh_data "
						  "FROM pg_publication p");
	else if (fout->remoteVersion >= 110000)
		appendPQExpBuffer(query,
						  "SELECT p.tableoid, p.oid, p.pubname, "
						  "p.pubowner, "
						  "p.puballtables, p.pubinsert, p.pubupdate, p.pubdelete, p.pubtruncate, false AS pubviaroot, false AS pubddl_database, false AS pubddl_table, false AS pubrefresh, false AS pubrefresh_data "
						  "FROM pg_publication p");
	else
		appendPQExpBuffer(query,
						  "SELECT p.tableoid, p.oid, p.pubname, "
						  "p.pubowner, "
						  "p.puballtables, p.pubinsert, p.pubupdate, p.pubdelete, false AS pubtruncate, false AS pubviaroot, false AS pubddl_database, false AS pubddl_table, false AS pubrefresh, false AS pubrefresh_data "
						  "FROM pg_publication p");
	...
	i_pubrefresh = PQfnumber(res, "pubrefresh");
	i_pubrefresh_data = PQfnumber(res, "pubrefresh_data");
	...
	for (i = 0; i < ntups; i++)
	{
		...
		pubinfo[i].pubddl_table =
			(strcmp(PQgetvalue(res, i, i_pubrefresh), "t") == 0);
		pubinfo[i].pubddl_table =
			(strcmp(PQgetvalue(res, i, i_pubrefresh_data), "t") == 0);
...
dumpPublication(Archive *fout, const PublicationInfo *pubinfo)
{
	...
	if (pubinfo->pubrefresh)
	{
		if (!first)
			appendPQExpBufferStr(query, ", ");

		appendPQExpBufferStr(query, "refresh");
		first = false;
	}
	...
	if (pubinfo->pubrefresh)
	{
		...
		appendPQExpBufferStr(query, "refresh data");
		first = false;
	}
